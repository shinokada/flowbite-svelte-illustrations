<script lang="ts">
	import { Label, Modal, uiHelpers } from 'svelte-5-ui-lib';
	import type { Component, Snippet } from 'svelte';
	import { twMerge } from 'tailwind-merge';
	import DynamicCodeBlockHighlight from './DynamicCodeBlockHighlight.svelte';
	import { Copy } from 'runes-webkit';
	import { isGeneratedCodeOverflow, toggleDarkMode, createImageVariants } from './helper';
...
`;
	...
	const illustModal = uiHelpers();
  let modalStatus = $state(false);
  const closeModal = illustModal.close;
  $effect(() => {
    modalStatus = illustModal.isOpen;
  });
	const updateIllust = (name: string) => {
    illustModal.toggle();
    selectedIllust = name;
		selectedIllust2 = toggleDarkMode(name);
  };
